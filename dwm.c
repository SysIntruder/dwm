...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			if (TEXTW(m->sel->name) > w) /* title is bigger thanthe width of the title rectangle, don't center */
				drw_text(drw, x, 0, w - 2 * sp, bh, lrpad / 2, m->sel->name, 0);
			else /* center window title */
				drw_text(drw, x, 0, w - 2 * sp, bh, (w - TEXTW(m->sel->name) - 2 * sp) / 2, m->sel->name, 0);
			if (m->sel->isfloating)
...
setup(void)
{
	...
	setsignal(SIGCHLD, SA_NOCLDSTOP | SA_NOCLDWAIT | SA_RESTART, SIG_IGN);
	...
	setsignal(SIGALRM, SA_NOCLDSTOP | SA_RESTART, sigalrm);

	#ifdef __linux__
	/* handle defined real time signals (linux only) */
	for (i = 0; i <LENGTH(blocks); i++) {
		if (blocks[i].signal)
			setsignal(SIGRTMIN + blocks[i].signal, SA_NOCLDSTOP | SA_RESTART, getsigcmds);
	}
	#endif /* __linux__ */
	
	/* pid as an environmental variable */
...
void
setsignal(int sig, int flags, void (*handler)(int unused))
{
	struct sigaction sa;
